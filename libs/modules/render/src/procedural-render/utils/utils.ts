...
export const getPixels = (resizeAuxNumber: number): Result[][] => {
  ...
  const array = Array(canvas.width).fill(
    Array(canvas.height).fill(undefined)
  ) as Result[][];
...
const convertToHex = (rgbCode: number[]): string => {
  return `#${rgbCode[0].toString(16)}${rgbCode[1].toString(
    16
  )}${rgbCode[2].toString(16)}`;
};
...
const pixelate = (resizeAuxNumber: number, imgUrl: string) => {
  ...
  context && context.drawImage(img, 0, 0, canvas.height, canvas.height);
